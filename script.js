...
const els={total:$('#totalCount'),due:$('#dueCount'),mastered:$('#masteredCount'),today:$('#todayCount'),studyBtn:$('#studyBtn'),shuffleBtn:$('#shuffleBtn'),newBtn:$('#newBtn'),emptyNewBtn:$('#emptyNewBtn'),exportBtn:$('#exportBtn'),importBtn:$('#importBtn'),importInput:$('#importInput'),editor:$('#editorPanel'),editorTitle:$('#editorTitle'),editorDesc:$('#editorDesc'),bulkToggle:$('#bulkToggle'),form:$('#cardForm'),editId:$('#editId'),question:$('#question'),singleAnswerField:$('#singleAnswerField'),answer:$('#answer'),singleImageField:$('#singleImageField'),imageInput:$('#imageInput'),imagePreviewWrap:$('#imagePreviewWrap'),imagePreview:$('#imagePreview'),removeImage:$('#removeImageBtn'),bulkArea:$('#bulkArea'),bulkImageInput:$('#bulkImageInput'),bulkItems:$('#bulkItems'),bulkCount:$('#bulkCount'),closeEditor:$('#closeEditorBtn'),cancelBtn:$('#cancelBtn'),search:$('#searchInput'),list:$('#cardList'),empty:$('#emptyState'),listSubtitle:$('#listSubtitle'),studyView:$('#studyView'),exitStudy:$('#exitStudyBtn'),fullscreen:$('#fullscreenBtn'),studyProgressText:$('#studyProgressText'),studyDueText:$('#studyDueText'),studyProgressBar:$('#studyProgressBar'),studyQuestion:$('#studyQuestion'),studyImage:$('#studyImage'),showAnswer:$('#showAnswerBtn'),answerArea:$('#answerArea'),studyAnswer:$('#studyAnswer'),ratingArea:$('#ratingArea'),toast:$('#toast'),themeBtn:$('#themeBtn')};

const DB_NAME='flashcards_app',STORE='cards',DAY=86400000,MINUTE=60000;
let db,cards=[],editingImage=null,bulkMode=false,bulkEntries=[],studyQueue=[],studyCurrent=null,sessionTotal=0,sessionDone=new Set(),shuffled=false,toastTimer;
...
const now=()=>Date.now(),isDue=c=>!c.nextReview||c.nextReview<=now(),sameDay=(a,b)=>new Date(a).toDateString()===new Date(b).toDateString();
...
function render(){
  ...
  filtered.sort((a,b)=>(isDue(b)-isDue(a))||((a.nextReview||0)-(b.nextReview||0))).forEach(c=>els.list.appendChild(cardRow(c)));
}
...
function cardRow(c){
  ...
  const media=c.image
    ?Object.assign(document.createElement('img'),{className:'thumb',src:c.image,alt:''})
...
function setBulkMode(on){
  ...
  els.bulkToggle.textContent=on?'✓ Modo lote ativado':'▦ Modo lote por imagens';
  els.singleAnswerField.classList.toggle('hidden',on);
  els.singleImageField.classList.toggle('hidden',on);
...
function openEditor(id=null){
  ...
  if(id){
    ...
    els.editorDesc.textContent='Altere a pergunta, resposta ou imagem deste cartão.';
...
function renderBulkEntries(){
  ...
  els.bulkCount.textContent=bulkEntries.length
    ?`${bulkEntries.length} ${bulkEntries.length===1?'imagem selecionada':'imagens selecionadas'}`
  ...
  bulkEntries.forEach((entry,index)=>{
    ...
    ta.rows=3;
    ta.maxLength=10000;
    ta.placeholder='Resposta para esta imagem...';
...
els.form.addEventListener('submit',async e=>{
  ...
  try{
    if(bulkMode&&!id){
      if(!bulkEntries.length)return toast('Adicione pelo menos uma imagem.');
      if(bulkEntries.some(x=>!x.answer.trim()))return toast('Preencha a resposta de todas as imagens.');
...
async function removeCard(id){
  ...
  if(!c||!confirm(`Excluir o flashcard “${c.question.slice(0,80)}${c.question.length>80?'…':''}”?`))return;
...
function startStudy(){
  ...
  if(!due.length){
    if(!confirm('Não há revisões pendentes. Quer estudar todos os cartões mesmo assim?'))return;
    due=[...cards];
  }
...
function nextStudyCard(){
  ...
  els.studyImage.classList.toggle('hidden',!studyCurrent.image);

  if(studyCurrent.image)els.studyImage.src=studyCurrent.image;
...
async function rateCard(rating){
  ...
  if(rating==='again'){
    ...
    c.interval=prevReviews===0
      ?1
      :prevReviews===1
        ?3
        :Math.max(2,Math.round(Math.max(1,interval)*c.ease));
    ...
    c.interval=prevReviews===0
      ...
      :prevReviews===1
        ?7
        :Math.max(4,Math.round(Math.max(1,interval)*c.ease*1.5));
...
document.addEventListener('keydown',e=>{
  ...
  if(e.key==='Escape'&&!document.fullscreenElement)closeStudy();
});
